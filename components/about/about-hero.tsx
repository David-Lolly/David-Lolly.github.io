...
function AvatarPanel({
  ...
  return (
    <div className="h-full rounded-[1.55rem] border border-stone-900/10 bg-white/55 p-2 shadow-sm dark:border-white/10 dark:bg-[#1f1f23] dark:shadow-[0_18px_50px_rgba(0,0,0,0.36)]">
      <button
        type="button"
        onClick={() => onOpenImage(avatar)}
        className="group block h-full w-full overflow-hidden rounded-[1.25rem] bg-[#fffdf3]/70 dark:bg-[#1f1f23]"
        aria-label={avatar.alt || avatar.caption || "\u6253\u5f00\u4e2a\u4eba\u7167\u7247"}
      >
        <img
          src={avatar.src}
          alt={avatar.alt || avatar.caption || "\u4e2a\u4eba\u7167\u7247"}
          className="block h-full w-full rounded-[1.25rem] object-cover transition duration-300 group-hover:scale-[1.01]"
        />
      </button>
    </div>
  );
...
function StatusPill({ currentStatus }: { currentStatus?: AboutHeroProps["currentStatus"] }) {
  ...
  return (
    <div className="inline-flex max-w-full items-center gap-3 rounded-full border border-[#3f6f53]/18 bg-[#3f6f53]/10 px-4 py-2 text-sm font-black text-[#315d43] shadow-[inset_0_1px_0_rgba(255,255,255,0.62)] md:text-base dark:border-emerald-400/25 dark:bg-emerald-400/10 dark:text-emerald-200 dark:shadow-none">
      <span className="h-2.5 w-2.5 shrink-0 rounded-full bg-[#3f6f53] dark:bg-emerald-300" />
...
function StatusMeta({ currentStatus }: { currentStatus?: AboutHeroProps["currentStatus"] }) {
  ...
  return (
    <p className="mt-4 text-sm font-black leading-6 text-stone-500 md:text-base dark:text-muted-foreground">
...
function FocusLine({ focus }: { focus: string[] }) {
  ...
  return (
    <div className="mt-5 flex flex-wrap items-center gap-x-2 gap-y-2 text-sm font-bold text-stone-600 dark:text-muted-foreground">
      {focus.map((item, index) => (
        <React.Fragment key={item}>
          <span className={index === 0 ? "text-[#8b6f47] dark:text-amber-300" : undefined}>{item}</span>
          {index < focus.length - 1 && <span className="text-stone-300 dark:text-white/35">{"\u00b7"}</span>}
        </React.Fragment>
      ))}
    </div>
  );
...
function MetricRow({ metrics }: { metrics: AboutHeroProps["metrics"] }) {
  ...
  return (
    <div className="mt-6 border-t border-stone-900/10 pt-5 dark:border-border">
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {metrics.map((item) => (
          <div key={item.label} className="flex min-w-0 flex-col items-center text-center">
            ...
            <div className="mt-2 truncate text-[10px] font-black uppercase tracking-[0.18em] text-[#8b6f47] dark:text-amber-300">
              {item.label}
            </div>
          </div>
        ))}
  ...
  return (
    <section className="relative overflow-hidden rounded-2xl border border-white/60 bg-white/45 p-3 shadow-[0_18px_55px_rgba(48,38,20,0.08)] backdrop-blur-xl sm:rounded-[2rem] sm:p-4 md:p-5 dark:border-white/10 dark:bg-[rgb(24,24,27)] dark:shadow-[0_24px_72px_rgba(0,0,0,0.42)]">
      <div className="relative grid gap-5 md:grid-cols-[16rem_1fr] md:items-stretch xl:grid-cols-[17.5rem_1fr]">
        ...
        <div className="rounded-[1.55rem] border border-white/55 bg-[#fffdf3]/58 p-5 shadow-sm backdrop-blur md:p-6 lg:px-8 dark:border-white/10 dark:bg-[#1f1f23] dark:shadow-[0_18px_50px_rgba(0,0,0,0.36)]">
          <div className="min-w-0">
            ...
            <p className="mt-6 max-w-4xl text-[15px] leading-8 text-stone-600 dark:text-muted-foreground">{profile.intro}</p>
